 ...
 *              支持驱动IC型号包括:ILI9341/NT35310/NT35510/SSD1963/ST7789/ILI9481/ILI9486/ILI9488/ST7796 等
 *
...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
	...
	if (lcddev.id==0X9486 || lcddev.id==0X7796) return r;   /* 直接读就可以 */
...
void lcd_set_cursor(uint16_t x, uint16_t y)
{
    ...
    {
		if(lcddev.dir==1&&lcddev.id==0x9481) x=lcddev.width-1-x;//横屏时处理
        lcd_wr_regno(lcddev.setxcmd);
        lcd_wr_data(x >> 8);
        lcd_wr_data(x & 0XFF);
        lcd_wr_regno(lcddev.setycmd);
...
void lcd_set_cursor9486_9488(uint16_t x, uint16_t y)
{
	lcd_wr_regno(lcddev.setxcmd); 
	lcd_wr_data(x>>8);	lcd_wr_data(x&0XFF); 
	lcd_wr_regno(lcddev.setycmd); 
...
void lcd_scan_dir(uint8_t dir)
{
    ...
    if ((lcddev.dir == 1 && lcddev.id != 0X1963 && lcddev.id!=0x9481) || (lcddev.dir == 0 && lcddev.id == 0X1963))
    {
        switch (dir)   /* 方向转换 */
        {
            case 0:
    ...
    /* 9341 & 7789 要设置BGR位 */
    if (lcddev.id == 0X9341 || lcddev.id == 0X7789 || lcddev.id == 0x9481 ||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796)
    {
        regval |= 0X08;
    }
	if(lcddev.id==0x9481) regval|=0x02;	//9481的BIT6和9341的反了 
    ...
    if (lcddev.id != 0X1963)   /* 1963不做坐标处理 */
    {
        ...
        else if(lcddev.id!=0x9481||lcddev.dir!=1) 
        {
            if (lcddev.width > lcddev.height)   /* 交换X,Y */
            {
                temp = lcddev.width;
...
void lcd_display_dir(uint8_t dir)
{
    ...
    if (dir == 0)       /* 竖屏 */
    {
        ...
        if (lcddev.id == 0X5310 ||lcddev.id==0x9481 ||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796)    /* 如果是5310 则表示是 320*480分辨率 */
        {
            lcddev.width = 320;
            lcddev.height = 480;
        }    
...
void lcd_init(void)
{
    ...
    if (lcddev.id != 0X9341 && lcddev.id!=0X9486&& lcddev.id!=0X9488 &&lcddev.id!=0x7796)    /* 不是 9341 , 尝试看看是不是 ST7789 */
    {
        lcd_wr_regno(0X04);
        lcddev.id = lcd_rd_data();      /* dummy read */
	...
	{
		lcd_ex_ili9481_reginit();   /* 执行ILI9481初始化 */ 
	}
	else if(lcddev.id==0x9486) 	//9486初始化
	{
		lcd_ex_ili9486_reginit();   /* 执行ILI9486初始化 */
	}
	else if(lcddev.id==0x9488) 	//9488初始化
	{
		lcd_ex_ili9488_reginit();   /* 执行ILI9488初始化 */
	}
